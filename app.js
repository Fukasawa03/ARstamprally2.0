...
var UI = (function () {
  ...
  function showModal(id) {
    var el = document.getElementById(id);
    if (el) { el.style.display = ''; el.classList.add('open'); }
  }
...
var AR = (function () {
  ...
  /* AR画面を表示するときにA-Frameをロードしてシーンを生成 */
  function startScene() {
    var wrapper = document.getElementById('ar-scene-wrapper');
    if (!wrapper) return;
    wrapper.innerHTML = '';

    // A-FrameとAR.jsを動的ロード（まだ読み込まれていない場合のみ）
    function buildScene() {
      ...
      var sceneHtml =
        '<a-scene embedded ' +
          ...
          'loading-screen="dotsColor:#c840ff; backgroundColor:#0d0121" ' +
      ...
      function attachMarkerEvents() {
        _stamps.forEach(function (s) {
          ...
          if (!marker) return;
          marker.addEventListener('markerFound', function () { handleDetect(s); });
        });
      }
      ...
      if (status) {
        ...
        var observer = new MutationObserver(function () {
          ...
          if (video) { status.textContent = ''; observer.disconnect(); }
        });
...
var App = {
  ...
  bindNav: function () {
    ...
    bind('btn-start', function () {
      UI.showModal('modal-start-confirm');
    });

    // スタート確認モーダル
    bind('start-confirm-ok', function () {
      UI.hideModal('modal-start-confirm');
      UI.showScreen('screen-map');
      State.startTimer();
    });
    bind('start-confirm-cancel', function () {
      UI.hideModal('modal-start-confirm');
    });
    bind('start-confirm-backdrop', function () {
      UI.hideModal('modal-start-confirm');
    });
...
document.addEventListener('DOMContentLoaded', function () {
  // A-FrameのデフォルトUI（ローディング丸・VRボタン等）を消す
  function hideAFrameUI() {
    var selectors = ['.a-loader-title', '.a-enter-vr', '.a-orientation-modal', '[data-aframe-inspector]'];
  ...
  // DOMContentLoaded後 + 少し遅延でも確認
  hideAFrameUI();
  setTimeout(hideAFrameUI, 500);
  setTimeout(hideAFrameUI, 2000);
